...
Having a basic understanding of the data, we can proceed to generate hypotheses for the data.
...
From the heatmap, we can analyze the type of properties that are up for sale within each district and compare whether certain property types tend to be built within a particular district. While there seems to be little difference in the number of count for terrace, semi-detached and detached houses across the districts, we can see that the number of condominiums up for sale is the highest in district 9 and that for apartments is in district 19. 
...
Also, from the geographical map, we observe that district 7, 8 and 9 tend to have high average psf while district 25 appear to have the lowest average psf. It is not surprising to see that most of the houses in district 9 are bigger since earlier we uncovered that there is a high concentration of condominiums up for sale in district 9; this alludes to suppliers of condominiums building houses that are larger than average to meet the needs of wealthier buyers. 
...
In the first boxplot, we can see that houses in district 1, 2 and, 9 are the most expensive as they have the highest average cost psf compared to the other districts. The second boxplot show the distribution of psf depending on the property type and we see that apartments and condominium are generally more expensive which explains why district 9 is expensive as it has largely condominiums. 

Even though district 9 has the highest number of counts of apartments, it is not as expensive compared to district 1, 2 & 9. Therefore, we then further breakdown and analyze the cost of each property type within each district. (Housing & Development Board (HDB) flats data are omitted because it has no information on district.) It was observed that the cost of psf for apartments in district 19 are generally lower than $2000 psf compared to district 1 & 2 where are the the apartments cost more than $2000 psf. Also even though district 2 may be the most expensive district on average, one should not assume that every property type is the most expensive there. For example, terrace houses are more expensive in district 4, 7 & 10, whereas condominiums are more expensive in district 9.
...
The area and logarithm of the asking price were plotted to uncover their relationship. From the graph, we observed that the correlation is highest among the property type – Apartment. However, in contrast with the HDB room types, the trend is not as strong. Hence, we cannot conclude that the hypothesis is applicable to all housing types.
